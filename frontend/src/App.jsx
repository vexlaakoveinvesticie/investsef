...
const DATA_SOURCES = [
  { id: "finnhub", name: "Finnhub", key: true, role: "Primárny", detail: "Real-time kotácie, ~60 req/min (kľúč na serveri)", status: "online" },
  { id: "yfinance", name: "Yahoo Finance (yfinance)", key: false, role: "Záloha 1", detail: "OHLCV + intraday sviečky, delay ~15 min", status: "online" },
  { id: "stooq", name: "Stooq (CSV)", key: false, role: "Záloha 2", detail: "Denné dáta 20+ rokov, bez API kľúča", status: "online" },
      ...
      {state === "done" && report && (
        <>
          <Panel title="AI Calibration Results" icon={CheckCircle2} right={<span className="text-[11px] text-secondary font-mono">threshold {report.threshold_used} · {report.timeframe}</span>}>
            ...
            {report.per_symbol_results && (
              <div className="mt-4">
                <div className="text-[10px] text-secondary font-mono mb-2">VÝSLEDKY PODĽA SYMBOLU</div>
                <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                  {Object.entries(report.per_symbol_results)
                    .sort((x, y) => y[1].pnl_pct - x[1].pnl_pct)
                    .map(([sym, st]) => (
                      <div key={sym} className="rounded-lg bg-raised p-2.5">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-semibold">{sym}</span>
                          <span className="text-xs font-mono font-semibold" style={{ color: st.pnl_pct >= 0 ? "var(--bull)" : "var(--bear)" }}>
                            {st.pnl_pct >= 0 ? "+" : ""}{st.pnl_pct}%
                          </span>
                        </div>
                        <div className="text-[10px] text-secondary font-mono mt-0.5">{st.trades} obch. · WR {st.win_rate_pct}%</div>
                      </div>
                    ))}
                </div>
                {report.trade_universe && (
                  <p className="text-[11px] text-secondary mt-2">Aktívne obchodované universe: {report.trade_universe.join(", ")}</p>
                )}
              </div>
            )}
          </Panel>
...
function DashboardTab({ assetKey, a, marketData }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <Panel title="Zdroj dát (failover reťazec)" icon={Database} className="lg:col-span-3">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          <div className="rounded-lg bg-raised p-3" style={{ border: "1px solid var(--bull-soft)" }}>
            <div className="flex items-center gap-1.5 mb-1"><Zap size={13} className="text-bull" /><span className="text-xs font-medium">1 · Finnhub</span></div>
            <div className="text-[10px] text-secondary font-mono">PRIMÁRNY · real-time kotácie · kľúč na serveri</div>
          </div>
          <div className="rounded-lg bg-raised p-3">
            <div className="flex items-center gap-1.5 mb-1"><CheckCircle2 size={13} className="text-accent" /><span className="text-xs font-medium">2 · Yahoo Finance</span></div>
            <div className="text-[10px] text-secondary font-mono">Záloha · sviečky/intraday · ~15 min delay</div>
          </div>
          <div className="rounded-lg bg-raised p-3">
            <div className="flex items-center gap-1.5 mb-1"><Database size={13} className="text-accent" /><span className="text-xs font-medium">3 · Stooq CSV</span></div>
            <div className="text-[10px] text-secondary font-mono">Záloha · denné dáta · bez kľúča</div>
          </div>
        </div>
        <p className="text-[11px] text-secondary mt-3 leading-relaxed">
          Používateľ webu <span className="text-primary">nezadáva žiadny API kľúč</span>. Systém vždy skúsi najprv Finnhub (real-time); ak endpoint alebo limit nedovolí, automaticky prejde na Yahoo a potom Stooq. Všetko sa cachuje na serveri.
        </p>
      </Panel>
    </div>
  );
  ...
  return (
    <div className="terminal min-h-screen font-sans">
      ...
      <div className="border-b border-hairline bg-raised/40 px-3 py-2 text-center text-[11px] text-secondary font-mono flex items-center justify-center gap-2">
        ...
        Reálne dáta: Finnhub (real-time) → Yahoo → Stooq · sekcie bez LIVE badge sú DEMO · nie je to investičné odporúčanie
      </div>
